...
> **최종 업데이트**: 2026-06-09 (쿠폰이벤트 섹션 파싱 3종 버그 수정)
...
"5월 이달의 쿠폰"                         → 쿠폰이벤트  ← COUPON_TITLE_RE 패턴으로 인식
"6. 승부 예측 이벤트!"                     → 승부예측    ← SECTION_RE(^\d+.) 패턴으로 인식
"1. 5월 이달의 쿠폰 보상"                  → 쿠폰이벤트  ← 동일 유형 연속 → 섹션 병합 시 스킵
"06.25_ Event"                          → 기타 (탭 헤더 — 섹션 아님)
"∎ 진행 기간 : ..."                       → 기타 (섹션 내용 — 섹션 시작 아님)

# 내부 처리 순서
parse_section_title(val)   # 패턴 우선순위:
    # 1. TAB_HEADER_RE: "06.11_ Event" → None
    # 2. SECTION_RE:    "^\d+\." → 반환
    # 3. TITLE_RE:      "이벤트 제목 :" → 추출
    # 4. COUPON_TITLE_RE: "^\d+월 이달의 쿠폰" → 반환  ← 신규 추가
...
> ⚠️ **연속 동일 유형 병합**: 쿠폰이벤트처럼 "5월 이달의 쿠폰"(B192)과 "1. 5월 이달의 쿠폰 보상"(B195)이 모두 `쿠폰이벤트`로 감지되는 경우, `section_rows`에서 **연속 동일 유형을 병합**하여 하나의 섹션으로 처리한다. 이를 통해 섹션 시작이 B192로 정확히 잡힌다.
...
**이벤트 유형별 특수 날짜 규칙** (`_fix_coupon_month_label` + `_fix_coupon_end_date`):

| 이벤트 유형 | 규칙 | 예시 |
|---|---|---|
| 쿠폰이벤트 | 월 라벨 갱신 (`N월 이달의 쿠폰`) | 260625 → 6월, 260702 → 7월 |
| 쿠폰이벤트 | 종료일 = 신규 탭이 속한 달의 말일 | 260625 → 06/30, 260702 → 07/31 |
| 쿠폰이벤트 | 시작일 역전 방지: 시작일 > 종료일이면 탭 기준일로 교정 | 07/09 > 06/30 → 06/25 |

```
처리 순서 (쿠폰이벤트 추가 시):
1. 이력 탭에서 섹션 복사
2. from_tab → new_tab 날짜 시프트 적용
3. _fix_coupon_month_label(): "5월 이달의 쿠폰" → "6월 이달의 쿠폰" (탭 월로 교체)
4. _fix_coupon_end_date(): 진행 기간 종료일을 해당 월 말일로 덮어쓰기
                           시작일이 종료일보다 크면 탭 기준일로 교정

결과 예시 (260625 탭):
  날짜 시프트 후: 07/09(목) 12:00 ~ 06/14(토) 23:59:59
  말일 적용 후:  07/09(목) 12:00 ~ 06/30(토) 23:59:59
  시작일 역전 수정 후: 06/25(목) 12:00 ~ 06/30(토) 23:59:59  ← 최종
```
