...
> **최종 업데이트**: 2026-06-04 (쿠폰이벤트 말일 자동 설정)
...
에이전트 시작 전 프로젝트를 선택하면 장르·마켓이 자동 주입된다.

| 프로젝트 | 장르 | 마켓 | 상태 |
|---|---|---|---|
| FB/GL | 야구 | 글로벌 | ✅ 설정 완료 |
| FB/JP | (설정 필요) | 일본 | ⚠️ 장르 미설정 |
| NC/KR | (설정 필요) | 한국 | ⚠️ 장르 미설정 |
| NC/GL | (설정 필요) | 글로벌 | ⚠️ 장르 미설정 |

- 설정은 `output/config/projects.json`에 영속 저장
- 미설정 프로젝트 선택 시 장르 설정 패널 표시 → 저장 후 진행
- 프로젝트 선택 없이도 진행 가능 (기존 방식 유지)
...
#### 섹션별 분리 파싱 (`_extract_section_text`)

사용자가 번호 형식으로 답변하는 경우 각 줄을 분리해서 컨텍스트별로 독립 판단한다.

```
1. 필수 이벤트 : 모두 진행        ← 이 줄의 "모두 진행"은 선택적/추가 판단에 영향 없음
2. 선택적 이벤트 : PVP 이벤트만 진행  ← selective_text
3. 추가 가능한 이벤트 : 모두 진행  ← absent_text
```

> ⚠️ **이전 버그**: 전체 메시지에서 "모두 진행" 검색 → 필수 이벤트 줄의 "모두 진행"이 선택적 이벤트 `keep_all=True`로 오판되어 제거 미실행. 섹션 분리로 해결.

#### 선택적 이벤트 판단 (selective_text 기준)

| 입력 예시 | 처리 결과 |
|---|---|
| "모두 진행" / "그대로" / "전체 진행" | 선택적 이벤트 전부 유지 |
| "pvp 이벤트만 진행" / "pvp만" | PvP 유지, 나머지 제거 |
| 특정 이벤트명 언급 | 언급된 것만 유지, 나머지 제거 |

#### 추가 가능 이벤트 판단 (absent_text 기준)

| 입력 예시 | 처리 결과 |
|---|---|
| "모두 진행" / "모두 적용" / "모두 추가" | 추가 가능 이벤트 전부 추가 |
| "전부 넣어" / "다 넣어줘" / "다 포함" | 전부 추가 |
...
**추가 이벤트 날짜 자동 시프트** (`_apply_date_map_to_rows`):
- 섹션 복사 직후 `from_tab → new_tab` 날짜 맵(`_auto_date_map`) 생성
- 복사된 행 범위에만 날짜 치환 적용 (ws 전체 미적용)
- `datetime`/`date` 객체: `date_map`으로 직접 변환
- `str`: `MM/DD` 패턴 regex 치환 (요일 텍스트 등 날짜 주변 문자는 유지)

```
예시) 260423 쿠폰이벤트 → 260625 탭에 추가 시
  원본: ∎ 진행 기간 : 05/07(목) 12:00 ~ 05/31(일) 23:59:59
  변환: ∎ 진행 기간 : 07/09(목) 12:00 ~ 08/02(일) 23:59:59
```

**이벤트 유형별 특수 날짜 규칙** (`_fix_coupon_end_date`):

| 이벤트 유형 | 규칙 | 예시 |
|---|---|---|
| 쿠폰이벤트 | 종료일 = 신규 탭이 속한 달의 말일 | 260625 → 06/30, 260702 → 07/31 |

```
처리 순서 (쿠폰이벤트 추가 시):
1. 이력 탭에서 섹션 복사
2. from_tab → new_tab 날짜 시프트 적용
3. _fix_coupon_end_date(): 진행 기간 종료일을 해당 월 말일로 덮어쓰기

결과 예시 (260625 탭):
  날짜 시프트 후: ~ 08/02(일) 23:59:59
  말일 적용 후:  ~ 06/30(일) 23:59:59  ← 6월 30일로 고정
```

> 이벤트 유형별 특수 규칙을 추가하려면 `_add_event_sections` 내 `if etype == "..."` 블록에 `_fix_*` 함수를 추가한다.
