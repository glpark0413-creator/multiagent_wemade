...
> **참조 설계서**: `game-localization-agent-design.md`
...
**실패 처리**:
- Google Sheets 접근 불가 → Excel 단독 사용 + 경고 표시
- 둘 다 실패 → 에스컬레이션
...
`output/game-localizer/translate_request.json`:
```json
{
  "source_text": "원문",
  "text_type": "dialogue",
...
`output/game-localizer/translate_result.json`:
```json
{
  "text_type": "dialogue",
...
**실패 처리**: 자동 재시도 최대 2회 → 초과 시 에스컬레이션 (문제 지점 명시 출력)
...
`output/game-localizer/session_buffer.json`에 항목 추가:
```json
{
  "session_id": "...",
  "entries": [
...
**성공 기준**: 유효한 xlsx/csv 파일 생성, 버퍼의 모든 항목 포함
